...
export default function Login(props) {
    ...
    const onSubmitHandler = (e) => {
        e.preventDefault()
        if (!isUserId) { return alert('이메일을 확인해주세요') }
        if (!isPassword) { return alert('비밀번호를 확인해주세요') }
        if (!isPasswordConfirm) { return alert('비밀번호가 일치하지 않습니다')}
        if (userName.length === 0) { return alert('이름을 입력해주세요') }
        if (userBirthDay.length === 0) { return alert('생일을 입력해주세요') }
        if (nickName.length === 0) { return alert('닉네임을 입력해주세요') }
        if (passwordConfirm.length === 0) { return alert('비밀번호 확인을 입력해주세요') }
    ...
    return (
        <div style={{margin: "auto", width:"500px", height:"100%", marginTop:"2vh", padding:"50px", boxShadow:"rgba(0, 0, 0, 0.1) 0px 4px 12px"}}>
            ...
            <p />
            <button onClick={onSubmitHandler}>{auth ? '회원가입' : '로그인' }</button>
            </form>
            <button onClick={onChangeHandler}>{auth ? '로그인' : '회원가입' }으로 가기</button>
